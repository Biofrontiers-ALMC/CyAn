classdef CyTrackLinker
    %TRACKLINKER  Links objects by overlapping areas to form tracks
    ...
    properties
        ...
        tracks = CellTrackArray; %Use this to hold the celltrack object
    ...
    methods
        ...
        function obj = initializeTracks(obj,tFrame,varargin)
            ...
            [paramNames, paramList] = obj.validateInput(varargin);
            
            numParams = numel(paramNames);
            
            for iParam = 1:numParams
                %Add properties to track
                obj.tracks = obj.tracks.addProperty(paramNames{iParam},true);
            end
            
            %Add the mother/daughter property
            obj.tracks = obj.tracks.addProperty('MotherIdx',false);
            obj.tracks = obj.tracks.addProperty('DaughterIdx',false);
%             obj.tracks = obj.tracks.addProperty('FrameDivided',false);
            
            numTracksToAdd = numel(paramList);
            
            for iTrack = 1:numTracksToAdd
                
                currInput = paramList{iTrack};
                currInput = [currInput, 'MotherIdx',NaN,'DaughterIdx',NaN];
                
                obj = obj.addTrackedObject(tFrame,currInput,0);
            end
    ...
    methods
        ...
        function [obj, newTrackIdx] = addTrackedObject(obj,tFrame,trackParam,didDivide)
            ...
            [obj.tracks, newTrackIdx] = obj.tracks.addTrack(tFrame,trackParam{:});
            
            if obj.options.TrackMitosis                            
                obj.tracks = obj.tracks.updateProperty(tFrame,newTrackIdx,'MotherIdx',NaN);
                obj.tracks = obj.tracks.updateProperty(tFrame,newTrackIdx,'DaughterIdx',NaN);
            end
            ...
            colPos = find(strcmp(trackParam,'Position'));
            colArea = find(strcmp(trackParam,'Area'));
            
            %Get location of Position and Area, then use these as the
            %values
            obj.trackingInfo(idxNewObject).Position = trackParam{colPos + 1};
            obj.trackingInfo(idxNewObject).Area = trackParam{colArea + 1};
        ...
        function obj = assignToTrack(obj,tFrame,varargin)
            ...
            %Get the areas and positions
            colPos = find(strcmp(varargin,'Position'));
            colArea = find(strcmp(varargin,'Area'));
            
            newPositions = varargin{colPos + 1};
            newAreas = varargin{colArea + 1};
            
            %Calculate the costMatrix
            costMat = obj.makeLinkMatrix(newPositions);
            ...
            for iM = 1:nExistingTracks
                ...
                if assignedValue > 0 && assignedValue <= nNewDetections
                    ...
                    %Update the track
                    obj = obj.updateTrackedObject(iM,tFrame,paramList{assignedValue});
                    
                else
                    %Now check if cell went out of bounds (then stop
                    %tracking) or if the cell might have gone behind
                    %another one - keep tracking but update the Kalman
                    %filters (when it's implemented)
            ...
            for iN = 1:nNewDetections
                ...
                if assignedValue > 0 && assignedValue <= nNewDetections
                    ...
                    if obj.options.TrackMitosis
                        [isMitosis, motherIdx] = obj.testForMitosis(paramList{assignedValue});
                        ...
                        if isMitosis
                            ...
                            if obj.tracks.Data(motherTrackIdx).StartFrame == tFrame
                                isMitosis = false;
                            end
                        end
                        ...
                        if isMitosis
                            ...
                            %Handle if mother track was somehow created
                            %this frame
                            
                            %Make two new tracks, with assigned mother
                            %cells
                            [obj, daughterIdx1] = obj.addTrackedObject(tFrame,paramList{assignedValue},1);
                            
                            %---Todo: A better way to recover the nearest neighbour index---%
                            
                            %Need to create a track with the NN information
                            [~,nnIdx] = min(obj.calcDistance(obj.trackingInfo(motherIdx).Position, newPositions));
                            [obj, daughterIdx2] = obj.addTrackedObject(tFrame,paramList{nnIdx},1);
                            
                            %Update mother track indices
                            obj.tracks = obj.tracks.updateProperty(tFrame,daughterIdx1,'MotherIdx',motherTrackIdx);
                            obj.tracks = obj.tracks.updateProperty(tFrame,daughterIdx2,'MotherIdx',motherTrackIdx);
                            
                            try
                            
                            %Remove the last frame from the mother track
                            obj.tracks = obj.tracks.removeFrame(...
                                motherTrackIdx,...
                                tFrame);
                            ...
                            obj.tracks = obj.tracks.updateProperty(tFrame,motherTrackIdx,'DaughterIdx',[daughterIdx1,daughterIdx2]);
                            ...
                            %Create a new track
                            obj = obj.addTrackedObject(tFrame,paramList{assignedValue},0);
                    ...
                    %Nothing, it's assigned to the auxiliary matrix
        ...
        function obj = updateTrackedObject(obj,idx,tFrame,paramListIn)
            
            %Update the celltrack object
            trackToUpdate = obj.trackingInfo(idx).Index;
            obj.tracks = obj.tracks.updateTrack(trackToUpdate,tFrame,paramListIn{:});
            
            %Update tracking info
            colPos = find(strcmp(paramListIn,'Position'));
            colArea = find(strcmp(paramListIn,'Area'));
            
            obj.trackingInfo(idx).Position = paramListIn{colPos + 1};
            obj.trackingInfo(idx).Area = paramListIn{colArea + 1};
        ...
        function trackOut = getTrack(obj,trackIdx)
            ...
            trackOut = obj.tracks.getTrack(trackIdx);
            
        end
        
        function [isMitosis, motherIdx] = testForMitosis(obj,paramList)
            ...
            colPos = find(strcmp(paramList,'Position'));
            colArea = find(strcmp(paramList,'Area'));
            
            currPos = paramList{colPos + 1};
            currArea = paramList{colArea + 1};
    ...
    methods (Static = true)
        ...
        function [paramNames, paramList] = validateInput(varargin)
            %VALIDATEINPUT  Validates and reshapes the input parameter list
            
            paramsIn = varargin{1};
            
            if rem(numel(paramsIn),2) ~= 0
                error('Input parameters must be in Property/Value pairs');
            end
            
            paramsIn = reshape(paramsIn,2,[]);
            
            %Make sure that 'position' and 'area' are present
            if ~any(strcmp(paramsIn(1,:),'Position'))
                error('Position must be present');
            end
            
            if ~any(strcmp(paramsIn(1,:),'Area'))
                error('Area must be present');
            end
            
            paramNames = {paramsIn{1,:}};
            
            numObjects = size(paramsIn{2,1},1);
            
            paramList = cell(1,numObjects);
            
            for iO = 1:numObjects
                
                for iParam = 1:numel(paramNames)
                    paramList{iO}{(iParam - 1) * 2 + 1} = paramNames{iParam};
   
                    try
                    paramList{iO}{(iParam - 1) * 2 + 2} = paramsIn{2,iParam}(iO,:);
                    catch
                        keyboard
                    end
                end
