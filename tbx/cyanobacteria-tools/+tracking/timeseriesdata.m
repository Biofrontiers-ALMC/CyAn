...
%         function obj = setPropertyTracking(obj, propName, isTracked)
%             %SETPROPERTYTRACKING  Set property tracking parameter
%             %
%             %  Use this to change property tracking option. 
%             %
%             %  NOTE: If going from a tracked to untracked property, you
%             %  will lose all but the most recent frame data.
%             
%             %Check that the input is an existing property
%             if propertyExists(propName)
%                 
%                 if obj.pinfo.(propName).isTracked && ~isTracked
%                     %If current property is changing from tracked to
%                     %untracked, keep only the latest data
%                     obj.pinfo.(propName).isTracked = false;
%                     
%                     for iT = 1:numel(obj.tracks)
%                         
%                         if iscell(obj.tracks(iT).(propName))
%                             obj.tracks(iT).(propName) = obj.tracks(iT).(propName){end};
%                         else
%                             obj.tracks(iT).(propName) = obj.tracks(iT).(propName)(end,:);
%                         end
%                         
%                     end
%                     
%                 elseif ~obj.pinfo.(propName).isTracked && isTracked
%                     %Going from untracked to tracked data, fill with
%                     %default property values.
%                                 
%                     obj.pinfo.(propName).isTracked = true;
%                     
%                     
%                     
%                     defaultStruct = obj.makeDefault(nFramesToAdd);
